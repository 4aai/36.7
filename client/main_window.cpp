...
MainWindowClient::MainWindowClient(QWidget *parent) : QMainWindow(parent), _ui(new Ui::MainWindowClient)
{
    _ui->setupUi(this);
    this->setWindowTitle("[CLIENT] CHAT QT");
    ...
    _ui->currentUserLabel->setText("<font color=red>Current user:  not loggedin</font>");
    _ui->chatWithTextEdit->hide();

    connect(_ui->registrationButton, &QPushButton::clicked, this, &MainWindowClient::openRegistrationWidget);
    ...
    connect(_socket, &QTcpSocket::disconnected, this, &MainWindowClient:: disconnect);
    //connect(_ui->closeButton, &QPushButton::clicked, this, &MainWindowClient:: close);
    connect(_ui->exitUserButton, &QPushButton::clicked, this, &MainWindowClient ::exitUser);
    ...
    connect(usersCB, &QComboBox::currentTextChanged, this,
    [this,usersCB] (QString login)
    {
        if (usersCB->currentIndex() != -1 && _ui->privateMessageRadioButton->isChecked())
        {
    ...
    {
        _ui->chatWithTextEdit->clear();
        if (usersCB_private->currentIndex() != -1 && _ui->privateMessageFromRadioButton->isChecked())
        {
    ...
    users)
{
    ...
    _ui->currentUserLabel->setText(QString("<font color=green>Current user: %1</font>").arg(login));
...
{
    _ui->currentUserLabel->setText(QString("<font color=red>Current user: %1 (in ban)</font>").arg(getCurLogin()));
    _ui->messageToAllRadioButton->setChecked(true);
    _ui->messageToAllRadioButton->setEnabled(false);
    _ui->privateMessageRadioButton->setEnabled(false);
    _ui->allPrivateMessagesRadioButton->setChecked(true);
    ...
    if (_ui->privateUsersComboBox->currentIndex() != -1)
    {
        ...
        else
        {
            ...
            for (auto& message : messages)
            {
                ...
                _ui->chatWithTextEdit->append(QString("<font color=blue>%1</font> write to <font color=magenta>%2</font>:").arg(user1).arg(user2) +
...
void MainWindowClient::ReceiveCommandMessageFromServer(QString cmd)
{
    ...
    {
    case 1:
        //QMessageBox(QMessageBox::Information, QObject::tr("Message"), QObject::tr("Message was send!"), QMessageBox::Ok).exec();
        break;
    case 2:
        //QMessageBox(QMessageBox::Information, QObject::tr("Message"), QObject::tr("Private message was send!"), QMessageBox::Ok).exec();
        break;
    case 3:
        QMessageBox(QMessageBox::Information, "Logout user", "You have logged out of the chat", QMessageBox::Ok).exec();
        break;
    case 4:
        QMessageBox(QMessageBox::Critical, "Error", "Error send message", QMessageBox::Ok).exec();
        break;
    default:
        break;
    }
...
void MainWindowClient::openLoginWidget()
{
    ...
    _loginWindow->setWindowFlags(Qt::CustomizeWindowHint | Qt::WindowTitleHint);
    _loginWindow->show();
}
...
void MainWindowClient::openRegistrationWidget()
{
    ...
    _registrationWindow->setWindowFlags(Qt::CustomizeWindowHint | Qt::WindowTitleHint);
...
void MainWindowClient::on_sendButton_clicked()
{
    ...
    if (_ui->messageToAllRadioButton->isChecked())
    {
        ...
        _ui->allMessagesTextEdit->append(
            QString("<font color=darkSlateBlue>%1</font> <font color=magenta>From Me</font>: ").arg(QTime::currentTime().toString())+
            QString("<font color=grey>%1</font>").arg(_ui->messageLineEdit->text()));
    ...
    if (_ui->privateMessageRadioButton->isChecked())
    {
        ...
        {
            _ui->chatWithTextEdit->append(QString("<font color=blue>%1</font> write to <font color=magenta>%1</font>: ").arg(getCurLogin()).arg(reciever) +
                                          QString("<font color=gray>%1</font>").arg(_ui->messageLineEdit->text()));
        }
 ...
 *
 *  147 \
 *  ...  > user's state for private message
 *  148 /
 *
 *  200 - user left chat
 *
...
void MainWindowClient::readData()
{
    ...
    case 100:
        _ui->allMessagesTextEdit->append(
            QString("<font color=green>%1 %2 join to Chat!</font>").arg(QTime::currentTime().toString()).arg(str.section(';',1)));

        _ui->usersOnlineListWidget->addItem(str.section(';',1));
        ind = _ui->usersComboBox->findText(str.section(';',1), Qt::MatchExactly);
        if (ind == -1)
            _ui->usersComboBox->addItem(str.section(';',1));

        ind = _ui->privateUsersComboBox->findText(str.section(';',1), Qt::MatchExactly);
        if (ind == -1)
    ...
    case 123:
        ...
        break;
    case 131:
        _ui->allMessagesTextEdit->append(QString("%1 <font color=blue>%1</font> write: ").arg(QTime::currentTime().toString()).arg(str.section(';',1,1)) +
                                         QString("<font color=grey>%1</font>").arg(str.section(';',2)));

        break;
    case 141:
        if (_ui->privateMessageFromRadioButton->isChecked() && _ui->privateUsersComboBox->currentText() == str.section(";",1,1))
        {
            _ui->chatWithTextEdit->append(QString("<font color=blue>%1</font> write to <font color=magenta>%2</font>: ").arg(str.section(";",1,1)).arg(getCurLogin()) +
                                          QString("<font color=gray>%1</font>").arg(str.section(';',2)));
        }
        _ui->privateMessagesTextEdit->append(QString("%1 Message from <font color=blue>%1</font>: ").arg(QTime::currentTime().toString()).arg(str.section(";",1,1))+
                                             QString("<font color=grey>%1</font>").arg(str.section(';',2)));
    ...
    case 147:
        if (_ui->usersComboBox->currentIndex() != -1)
            _ui->usersStateLabel->setText("<font color=green>User online</font>");

        break;
    case 148:
        if(_ui->usersComboBox->currentIndex() != -1)
            _ui->usersStateLabel->setText("<font color=red>User offline</font>");

        break;
    case 200:
        _ui->allMessagesTextEdit->append(QString("<font color=red>%1 %2 left the Chat!</font>").arg(QTime::currentTime().toString()).arg(str.section(';',1)));
        currentItem = _ui->usersOnlineListWidget->findItems(str.section(';',1), Qt::MatchExactly)[0];
        delete _ui->usersOnlineListWidget->takeItem(_ui->usersOnlineListWidget->row(currentItem));

        break;
    case 300:
        _ui->allMessagesTextEdit->append(QString("<font color=red>%1 You are in BAN!</font>").arg(QTime::currentTime().toString()));
        BanUser();

        break;
    case 301:
        _ui->allMessagesTextEdit->append(QString("<font color=red>%1 User with login %2 sent to ban!</font>").arg(QTime::currentTime().toString()).arg(str.section(';',1)));

        break;
    case 302:
        _ui->allMessagesTextEdit->append(QString("<font color=green>%1 You are UNBAN!</font>").arg(QTime::currentTime().toString()));
        EnterUserInChat(str.section(';',1,1),str.section(';',2));

        break;
    case 303:
        _ui->allMessagesTextEdit->append(QString("<font color=green>%1 User with login %2 Unban!</font>").arg(QTime::currentTime().toString()).arg(str.section(';',1)));
