...
auto SQLDB::OpenDB()->std::pair<bool, QString>
{
    if (!_db.open())
    {
        QMessageBox(QMessageBox::Warning, "Error", _db.lastError().text(), QMessageBox::Ok).exec();
        return std::make_pair(false, _db.lastError().text());
    }
    else
        return std::make_pair(true, "The Connection to the database was successful!");
}
...
auto SQLDB::CreateTablePrivateMessages()->std::pair<bool, QString>
{
    QSqlQuery query;
    if(!query.exec(
        "create table if not exists private_messages (id serial "
        "primary key, from_id integer references users(id), to_id integer "
        "references users(id), text varchar(1024));"
            ))
        return std::make_pair(false, query.lastError().text());
    else
        return std::make_pair(true, "Table 'private_messages' is created or finded!");
}
...
auto SQLDB::CreateTableAllMessages()->std::pair<bool, QString>
{
    QSqlQuery query;
    if(!query.exec(
        "create table if not exists to_all_messages (id serial "
        "primary key, from_id integer references users(id), "
        "text varchar(1024));"
            ))
        return std::make_pair(false, query.lastError().text());
    else
        return std::make_pair(true, "Table 'to_all_messages' is created or finded!");
}
    ...
    if (query.exec(str))
        while (query.next())
        {
            message = getUserById(query.value(0).toString()) + ";" +
                      getUserById(query.value(1).toString()) + ";" +
