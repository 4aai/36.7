...
MainWindowServer::MainWindowServer(QWidget *parent) : QMainWindow(parent), _ui(new Ui::MainWindowServer)
{
    _ui->setupUi(this);
    this->setWindowTitle("[SERVER] CHAT QT");
    _ui->portLineEdit->setText("1337");
    ...
    if (result.first)
    {
        _ui->informationFromClientTextEdit->append(
            ("<font color=darkGreen>%1 " + result.second + "</font>").arg(QTime::currentTime().toString()));
        ...
        else
        {
            ...
            _ui->informationFromClientTextEdit->append(
                ("<font color=darkRed>%1 " + result.second + "</font>").arg(QTime::currentTime().toString()));
        }
    ...
    connect(_server,&QTcpServer::newConnection,this, &MainWindowServer::newConnect);
    connect(_ui->allUsersRadioButton, &QRadioButton::toggled, this, [this] (const auto on)
    {
        if (on)
        {
    ...
    connect(_ui->usersListWidget, &QListWidget::itemClicked, this, [this](QListWidgetItem *item)
    {
    ...
    connect(_ui->usersOnlineRadioButton, &QRadioButton::toggled, this, [this](const auto on)
    {
        if (on)
        {
            for (auto& user : _users)
...
void MainWindowServer::newConnect()
{
    ...
    connect(clientSocket, &QTcpSocket::disconnected, this, &MainWindowServer::disconnect);
    _users.push_back(user);
    _ui->clientCountLineEdit->setText(QString::number(_users.size()));
    _ui->informationFromClientTextEdit->append(
        QString("<font color=darkGreen>%1 New client connect from port number %2</font>").arg(QTime::currentTime().toString()).arg(QString::number(port)));
}
...
void MainWindowServer::disconnect()
{
    ...
    for (auto& user : _users)
    {
        ...
        if (currentSocket == clientSocket)
        {
            ...
            {
                QListWidgetItem* currentItem = _ui->usersListWidget->findItems(user->getName(), Qt::MatchExactly)[0];
                ...
                if (user->isInBan() && _ui->inBanRadioButton->isChecked())
                    delete _ui->usersListWidget->takeItem(_ui->usersListWidget->row(currentItem));
            ...
            _ui->clientCountLineEdit->setText(QString::number(_users.size()));
            _ui->informationFromClientTextEdit->append(
                QString("<font color=darkRed>%1 Client from port number %2 disconnect</font>").arg(QTime::currentTime().toString()).arg(user->getPort()));
            break;
        }
    }
}
...
void MainWindowServer::readData()
{
    ...
    case 110:
        _ui->informationFromClientTextEdit->append(
            QString("%1 Requested command from client: Registration").arg(QTime::currentTime().toString()));
    ...
    case 120:
        _ui->informationFromClientTextEdit->append(
            QString("%1 Requested command from client: Login").arg(QTime::currentTime().toString()));
    ...
    case 130:
        _ui->informationFromClientTextEdit->append(
            QString("%1 Requested command from client: Send message to all").arg(QTime::currentTime().toString()));

        _ui->allMessagesTextEdit->append(
           QString("%1 <font color=magenta>%2</font> write:").arg(QTime::currentTime().toString()).arg(getClientBySocket(currentSocket)->getName())+
           QString("<font color=dimGray>%1</font>").arg(str.section(';',1)));
    ...
    case 140:
        _ui->informationFromClientTextEdit->append(
            QString("%1 Requested command from client: Send private message").arg(QTime::currentTime().toString()));
    ...
    case 141:
        _ui->informationFromClientTextEdit->append(
            QString("%1 Requested command from client: Get chat between two users").arg(QTime::currentTime().toString()));
    ...
    case 145:
        _ui->informationFromClientTextEdit->append(
            QString("%1 Requested command from client: Send list of users").arg(QTime::currentTime().toString()));
    ...
    case 146:
        _ui->informationFromClientTextEdit->append(
            QString("%1 Requested command from client: User is online or not").arg(QTime::currentTime().toString()));
    ...
    case 200:
        _ui->informationFromClientTextEdit->append(
           QString("%1 Requested command from client: Logout").arg(QTime::currentTime().toString()));
...
void MainWindowServer::SendMessageToAllButOne(QByteArray message, QTcpSocket *socket)
{
...
void MainWindowServer::Registration(QTcpSocket* socket, QString name, QString login, QString password)
{
    ...
    case 0:
        message = QTime::currentTime().toString() +
                  " User with login " + login + " not added in database! Registration failed...";
    ...
    case 1:
        message = QTime::currentTime().toString() +
                  " User with login " + login + " add in database! Registration success...";

        getClientBySocket(socket)->setState(true);
        getClientBySocket(socket)->setName(login);
        _usersOnline++;
        _ui->authUsersLineEdit->setText(QString::number(_usersOnline));
        _ui->allUsersLineEdit->setText(QString::number((_ui->allUsersLineEdit->text().toInt())+1));
        _ui->informationFromClientTextEdit->append(message);

        if (_ui->allUsersRadioButton->isChecked() || _ui->usersOnlineRadioButton->isChecked())
    ...
    case 2:
        message = QTime::currentTime().toString() +
                  " User with login " + login + " is already exists";
...
void MainWindowServer::LogIn(QTcpSocket *socket, QString login, QString password)
{
    ...
    {
        message = QTime::currentTime().toString() +
                  " The user with login " + login + " was authenticated";
    ...
    {
        message = QTime::currentTime().toString() +
                  " The user with login " + login + " is already online";
        _ui->informationFromClientTextEdit->append(message);
        socket->write("123;");
    }
    else
    {
        message = QTime::currentTime().toString() +
                  " The user with login " + login + " was not authenticated";
        _ui->informationFromClientTextEdit->append(message);
        socket->write("122");
    }
}
...
{
    _ui->informationFromClientTextEdit->append(
        QString("<font color=darkRed>%1 The user with login %2 logged out the chat</font>").arg(QTime::currentTime().toString()).arg(getClientBySocket(socket)->getName()));

    _ui->authUsersLineEdit->setText(QString::number(--_usersOnline));
    if (_ui->usersOnlineRadioButton->isChecked())
    {
        QListWidgetItem* currentItem = _ui->usersListWidget->findItems(getClientBySocket(socket)->getName(), Qt::MatchExactly)[0];
        delete _ui->usersListWidget->takeItem(_ui->usersListWidget->row(currentItem));
    }
...
{
    QString message;
    bool res = _dataBase->AddMessage(getClientBySocket(socket)->getName(), text);
    if (res)
    {
        for (auto i = 0; i < _users.size(); ++i)
        {
...
void MainWindowServer::SendPrivateMessage(QTcpSocket *socket, QString to, QString text)
{
    QString message;
    bool res = _dataBase->AddPrivateMessage(getClientBySocket(socket)->getName(), to, text);
    if (res)
    {
        for (auto& user : _users)
            if (user->getName() == to)
...
{
    if (_ui->connectButton->text() == QString("Start server"))
    {
        qint16 port = _ui->portLineEdit->text().toInt();
        if (!_server->listen(QHostAddress::Any, port))
        {
            QMessageBox(QMessageBox::Critical, "Error", _server->errorString(), QMessageBox::Ok).exec();
            return;
        }
        _ui->connectButton->setText("Stop server");
        ...
        _ui->allUsersLineEdit->setText(QString::number(_dataBase->getAllUsers().size()));
        ...
        for (const auto& message : messages)
        {
            ...
            _ui->allMessagesTextEdit->append(QString("<font color=blue>%1</font> write:").arg(user1) +
                                             QString("<font color=gray>%1</font>").arg(text));
        }
    }
    else
    {
        ...
        _server->close();
        _ui->connectButton->setText("Start server");
...
void MainWindowServer::on_showChatPushButton_clicked()
{
    ...
    if (messages.isEmpty())
        ...
        for (const auto& message : messages)
        {
            ...
            QMap<QString, QString> colorSelect;
            colorSelect[user1] = "blue";
            colorSelect[user2] = "magenta";
            _ui->privateMessagesTextEdit->append(
                QString("[<font color=%1>%2</font> write to <font color=%3>%4</font>]: ")
                    .arg(colorSelect[sender]).arg(sender).arg(colorSelect[reciever]).arg(reciever) +
                QString("<font color=gray>%1</font>").arg(text));
        }
}
    ...
    if(_ui->banUserPushButton->text() == QString("Ban user"))
    {
        ...
        for (auto& user : _users)
            if (user->getName() == client)
            {
                ...
                SendMessageToAllButOne(message.toUtf8(), user->getSocket());
                _ui->informationFromClientTextEdit->append(
                    QString("<font color=darkRed>%1 Client with login %2 sent to ban</font>").arg(QTime::currentTime().toString()).arg(user->getName()));
    ...
    else
    {
        ...
        for (auto& user : _users)
            if (user->getName() == client)
            {
                ...
                QString message;
                _ui->informationFromClientTextEdit->append(
                    QString("<font color=darkGreen>%1 Client with login %2 unban</font>").arg(QTime::currentTime().toString()).arg(user->getName()));
...
void MainWindowServer::on_allUsersRadioButton_clicked()
{
